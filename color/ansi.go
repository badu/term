...
// ansi is a io.Writer which translates ANSI escape codes into color tags.
type ansi struct {
	io.Writer
	...
	attributes                    string        // The buffer's current text attributes (a attribute string).

	// The current state of the parser. One of the ansi constants.
	state int
}

// ANSIWriter returns an io.Writer which translates any ANSI escape codes written to it into color tags. Other escape codes don't have an effect and are simply removed.
// The translated text is written to the provided  writer.
...
// with color tags and returns the resulting string.
