...
// The unicode properties. Only the ones needed in the context of this package are included.
const (
	prAny = iota
	prPreprend
	prCR
...
func codePoints() [][3]int {
	codePointsInit.Do(func() {
		codePointsData = [][3]int{
...
var (
	codePointsInit sync.Once
	codePointsData [][3]int
)

// property returns the Unicode property value (see constants) of the given code point.
// Note that the mutext is locked by the caller, Graphemes.Next method
func property(r rune) int {
	// Maps code point ranges to their properties.
	// In the context of this package, any code point that is not contained may map to "prAny".
	// The code point ranges in this slice are numerically sorted.
	//
	// These ranges were taken from http://www.unicode.org/Public/UCD/latest/ucd/auxiliary/GraphemeBreakProperty.txt
	// as well as https://unicode.org/Public/emoji/latest/emoji-data.txt
	// ("Extended_Pictographic" only) on March 11, 2019.
