...
// TBytes returns a transformer that replaces all instances of old with new.
...
// TString returns a transformer that replaces all instances of old with new.
// Unlike strings.Replace, empty old values don't match anything.
func TString(old, new string) Transformer {
	return TBytes([]byte(old), []byte(new))
}
