package info

import (
	"bytes"
	"testing"
	"time"

	"github.com/badu/term"
)

// This info entry is a stripped down version from
// xterm-256color, but I've added some of my own entries.
var testTerminfo = &Term{
	Name:      "simulation_test",
	Columns:   80,
	Lines:     24,
	Colors:    256,
	Bell:      "\a",
	Blink:     "\x1b2ms$<20>something",
	Reverse:   "\x1b[7m",
	SetFg:     "\x1b[%?%p1%{8}%<%t3%p1%d%e%p1%{16}%<%t9%p1%{8}%-%d%e38;5;%p1%d%;m",
	SetBg:     "\x1b[%?%p1%{8}%<%t4%p1%d%e%p1%{16}%<%t10%p1%{8}%-%d%e48;5;%p1%d%;m",
	AltChars:  "``aaffggiijjkkllmmnnooppqqrrssttuuvvwwxxyyzz{{||}}~~",
	Mouse:     "\x1b[M",
	MouseMode: "%?%p1%{1}%=%t%'h'%Pa%e%'l'%Pa%;\x1b[?1000%ga%c\x1b[?1003%ga%c\x1b[?1006%ga%c",
	SetCursor: "\x1b[%i%p1%d;%p2%dH",
	PadChar:   "\x00",
}

func TestTerminfoExpansion(t *testing.T) {
	ti := testTerminfo
	buf := bytes.NewBuffer(nil)
	ti.Init(buf, &term.Size{Width: ti.Columns, Height: ti.Lines})
	// Tests %i and basic parameter strings too
	p := ti.TParam(ti.SetCursor, 9, 7)
	if p != "\x1b[10;8H" {
		t.Errorf("GoTo expansion failed : %#v", p)
	}

	// This tests some conditionals
	if ti.TParam("A[%p1%2.2X]B", 47) != "A[2F]B" {
		t.Error("TParam conditionals failed")
	}

	// Color tests.
	if ti.TParam(ti.SetFg, 7) != "\x1b[37m" {
		t.Error("SetFg(7) failed")
	}
	if ti.TParam(ti.SetFg, 15) != "\x1b[97m" {
		t.Error("SetFg(15) failed")
	}
	if ti.TParam(ti.SetFg, 200) != "\x1b[38;5;200m" {
		t.Error("SetFg(200) failed")
	}

	if ti.TParam(ti.MouseMode, 1) != "\x1b[?1000h\x1b[?1003h\x1b[?1006h" {
		t.Error("Enable mouse mode failed")
	}
	if ti.TParam(ti.MouseMode, 0) != "\x1b[?1000l\x1b[?1003l\x1b[?1006l" {
		t.Error("Disable mouse mode failed")
	}
}

func TestTerminfoDelay(t *testing.T) {
	ti := testTerminfo
	buf := bytes.NewBuffer(nil)
	ti.Init(buf, &term.Size{Width: ti.Columns, Height: ti.Lines})
	now := time.Now()
	ti.PutBlink(buf)
	then := time.Now()
	s := string(buf.Bytes())
	if s != "\x1b2mssomething" {
		t.Errorf("Term delay failed: %s", s)
	}
	if then.Sub(now) > time.Millisecond*50 {
		t.Error("Too late delay")
	}
}

func BenchmarkSetFgBg(b *testing.B) {
	ti := testTerminfo

	for i := 0; i < b.N; i++ {
		ti.TParam(ti.SetFg, 100, 200)
		ti.TParam(ti.SetBg, 100, 200)
	}
}
